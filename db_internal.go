...
func (db *DB) maybeCompact() error {
	...
	// Open fresh readers for compaction
	readers := make([]*SSTableReader, len(level0))
	for i, path := range level0 {
	...
	// Merge into a new level-1 SSTable
	...
	// Close existing readers and remove old level-0 files
	for _, sst := range db.sstables {
		sst.Close()
	}
	for _, path := range level0 {
		os.Remove(path)
	}
	db.nextSeq++

	// Reload all SSTables from disk
	db.sstables = nil
	return db.loadSSTables()
}

// level0SSTables returns sorted paths of all level-0 SSTable files.
